...
import static pl.damian.producer.infrastructure.OfferMessageConst.Topics.case_1_config_12;

@Slf4j
@RequiredArgsConstructor
@FieldDefaults(level = AccessLevel.PRIVATE, makeFinal = true)
public class KafkaEventPublisherCase1Config12 implements EventPublisher<InternalEvent> {

    KafkaTemplate<String, InternalEvent> kafkaTemplateCase1Config12;
    ...
    public void send(InternalEvent event) {
        ...
        try {
            ...
            for (int i = 0; i < number.intValue(); i++) {
                final var producerRecord = new ProducerRecord<>(case_1_config_12, event.getKey(), event);
                final var sendResult = kafkaTemplateCase1Config12.send(producerRecord);
                kafkaTemplateCase1Config12.flush();
                sendResult.get();
            }
...
//            log.info("Send " + number.divide(new BigInteger(String.valueOf(timeElapsed))).multiply(new BigInteger("1000")) + " messages per second");
            log.info("Message was sent CASE1CONFIG12");
